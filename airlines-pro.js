...
function admin_user() {
  ...
  if (mayus === "USER") {
    ...
    alert(
      "Lo sentimos, ha ingresado un valor incorrecto. Vuelva a ejecutar la funcion"
    );
  }
}
...
function crearVuelo() {
  if (flights.length < 15) {
    alert(
      "Para añadir un vuelo deberas rellenar los datos en las siguientes ventanas:"
    );
    ...
    let scale1 = Boolean(
      parseInt(
        prompt(
          "En caso de que sea un vuelo con escala indique '1', de lo contrario '0' "
        )
      )
    );
    ...
    alert("No se pueden agregar mas de 15 vuelos");
  }
    ...
    switch (prec1.toLowerCase()) {
      case "mayor":
        ...
        for (let i = 0; i < mayor_aux1.length; i++) {
          console.log(
            `-- Id ${mayor_aux1[i].id} -- El vuelo con origen: ${mayor_aux1[i].from}, y destino: ${mayor_aux1[i].to} tiene un coste de ${mayor_aux1[i].cost} € y realiza, al menos, una escala.`
          );
        }
      ...
      case "igual":
        ...
        for (let i = 0; i < flights.length; i++) {
          if (flights[i].cost == prec2) {
            console.log(
              `-- Id ${flights[i].id} -- El vuelo con origen: ${flights[i].from}, y destino: ${flights[i].to} tiene un coste de ${flights[i].cost} € y realiza, al menos, una escala.`
            );
          }
        }
        alert("Consulta los resultados en la consola.");
